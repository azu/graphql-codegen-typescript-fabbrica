...
export type DefaultFactoryConfig = {
    factoryFile: string;
} & Config;
...
export function validateConfig(rawConfig: unknown): asserts rawConfig is RawConfig {
    ...
    if ('fakeOutputPath' in rawConfig && typeof rawConfig['fakeOutputPath'] !== 'string') {
        throw new Error('`options.fakeFile` must be a string');
    }
}
...
export function normalizeConfig(rawConfig: RawConfig): Config {
    return {
        ...
        convert: rawConfig.namingConvention
            ...
            : convertFactory({}),
        fakeOutputPath: rawConfig.fakeOutputPath,
    };
}
